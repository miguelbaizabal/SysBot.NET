...
public sealed class SysCord<T> where T : PKM, new()
{
    ...
    public SysCord(PokeBotRunner<T> runner, ProgramConfig config)
    {
        ...
        {
            LogUtil.LogText($"Conexión con Discord perdida. Motivo: {exception?.Message ?? "Desconocido"}");
    ...
    private async Task ReconnectAsync()
    {
        ...
        try
        {
            ...
            for (int i = 0; i < maxRetries; i++)
            {
                ...
                {
                    LogUtil.LogText("Intento de reconexión cancelado.");
                    return;
                }
                ...
                try
                {
                    ...
                    {
                        LogUtil.LogText("Cliente reconectado automáticamente.");
                    ...
                    {
                        LogUtil.LogText("Esperando reconexión automática...");
            ...
            LogUtil.LogText("Error al reconectar después de los intentos máximos. Reiniciando el bot...");
            ...
            try
            {
                ...
                // Restart the bot
                await _client.StartAsync().ConfigureAwait(false);
                LogUtil.LogText("Bot reiniciado exitosamente.");
            }
            catch (Exception ex)
            {
                LogUtil.LogText($"Error al reiniciar el bot: {ex.Message}");
            }
        }
        ...
        {
            LogUtil.LogText($"Error inesperado en ReconnectAsync: {ex.Message}");
        }
        finally
        {
    ...
    public async Task AnnounceBotStatus(string status, EmbedColorOption color)
    {
        ...
        var embed = new EmbedBuilder()
            .WithTitle("Reporte de Estado del Bot")
        ...
        foreach (var channelId in SysCordSettings.Manager.WhitelistedChannels.List.Select(channel => channel.ID))
        {
            try
            {
                ...
                if (textChannel != null)
                {
                    ...
                    if (SysCordSettings.Settings.ChannelStatus)
                    {
                        ...
                        {
                            LogUtil.LogInfo("SysCord", $"No se puede actualizar el nombre del canal para {channelId}: Permisos insuficientes para gestionar canales");
                        }
                        catch (HttpException ex) when (ex.DiscordCode == DiscordErrorCode.RequestEntityTooLarge)
                        {
                            LogUtil.LogInfo("SysCord", $"No se puede actualizar el nombre del canal para {channelId}: Límite de solicitudes alcanzado");
                        }
                        catch (Exception ex)
                        {
            ...
            {
                LogUtil.LogInfo("SysCord", $"AnnounceBotStatus: Excepción en canal {channelId}: {ex.Message}");
            }
        }
    }
    ...
    private void InitializeRecoveryNotifications()
    {
        ...
        LogUtil.LogInfo("Notificaciones de recuperación inicializadas para Discord", "Recovery");
    }
    ...
    public async Task MainAsync(string apiToken, CancellationToken token)
    {
        ...
        catch (OperationCanceledException)
        {
            ...
            LogUtil.LogText("MainAsync: El bot está desconectándose debido a la cancelación...");
            await AnnounceBotStatus("Desconectado", EmbedColorOption.Red);
    ...
    private async Task HandleMessageAsync(SocketMessage arg)
    {
        ...
        try
        {
            ...
            if (SysCordSettings.Settings.ReplyToThanks && (thanksText.Contains("thank") || thanksText.Contains("gracias")))
            {
            ...
            else if (content.Length > 1 && content[0] != correctPrefix[0])
            {
                ...
                {
                    await SafeSendMessageAsync(msg.Channel, $"¡Prefijo incorrecto! El comando correcto es **{correctPrefix}{command}**").ConfigureAwait(false);
                    return;
                }
            }
        ...
        {
            await Log(new LogMessage(LogSeverity.Warning, "Command", $"Permisos insuficientes para manejar un mensaje en el canal {arg.Channel.Name}")).ConfigureAwait(false);
        }
        catch (Exception ex)
        {
        ...
        finally
        {
            ...
            if (stopwatch.ElapsedMilliseconds > 1000) // Log if processing takes more than 1 second
            {
                await Log(new LogMessage(LogSeverity.Warning, "Gateway",
                    $"Un controlador de MessageReceived está bloqueando la tarea del gateway. " +
                    $"Método: HandleMessageAsync, Tiempo de Ejecución: {stopwatch.ElapsedMilliseconds}ms, " +
    ...
    private async Task LoadLoggingAndEcho()
    {
        ...
        await Log(new LogMessage(LogSeverity.Info, "LoadLoggingAndEcho()", "¡Canales de Logging y Echo cargados!")).ConfigureAwait(false);
    ...
    private async Task<bool> TryHandleCommandAsync(SocketUserMessage msg, SocketCommandContext context, int pos)
    {
        try
        {
            ...
            {
                await SysCord<T>.SafeSendMessageAsync(msg.Channel, "Estás baneado de usar este bot.").ConfigureAwait(false);
                return true;
            }
            ...
            {
                await SysCord<T>.SafeSendMessageAsync(msg.Channel, "No tienes permiso para usar este comando.").ConfigureAwait(false);
                return true;
            }
            ...
            await Log(new LogMessage(LogSeverity.Info, "Command", $"Ejecutando comando desde {guild}#{msg.Channel.Name}:@{msg.Author.Username}. Contenido: {msg}")).ConfigureAwait(false);
        ...
        {
            await Log(new LogMessage(LogSeverity.Error, "Command", $"Error ejecutando comando: {ex.Message}", ex)).ConfigureAwait(false);
            return false;
        }
    }
    ...
    private static async Task SafeSendMessageAsync(IMessageChannel channel, string message)
    {
        ...
        {
            await Log(new LogMessage(LogSeverity.Warning, "Command", $"Permisos insuficientes para enviar mensaje en el canal {channel.Name}")).ConfigureAwait(false);
        }
        catch (Exception ex)
        {
            await Log(new LogMessage(LogSeverity.Error, "Command", $"Error enviando mensaje: {ex.Message}", ex)).ConfigureAwait(false);
        }
    }
}
