...
public class RemoteControlModule<T> : ModuleBase<SocketCommandContext> where T : PKM, new()
{
    ...
    public async Task ClickAsync(SwitchButton b)
    {
        ...
        if (bot == null)
        {
            await ReplyAsync($"Ningún bot está disponible para ejecutar tu comando: {b}").ConfigureAwait(false);
            return;
        }
    ...
    private async Task SetScreenForAllBots(bool on)
    {
        ...
        if (bots.Count == 0)
        {
            await ReplyAsync("No hay bots conectados actualmente.").ConfigureAwait(false);
            return;
        }
        ...
        await ReplyAsync($"Estado de la pantalla establecido a {(on ? "Encendida" : "Apagada")} para {successCount} de {bots.Count} bots.").ConfigureAwait(false);
    }

    [Command("setStick")]
    [Summary("Establece el joystick en la posición especificada.")]
    [RequireRoleAccess(nameof(DiscordManager.RolesRemoteControl))]
    public async Task SetStickAsync(SwitchStick s, short x, short y, ushort ms = 1_000)
    {
        ...
        if (bot == null)
        {
            await ReplyAsync($"Ningún bot está disponible para ejecutar tu comando: {s}").ConfigureAwait(false);
            return;
        }
    ...
    private async Task SetStickAsyncImpl(SwitchStick s, short x, short y, ushort ms, BotSource<PokeBotState> bot)
    {
        ...
        {
            await ReplyAsync($"Joystick desconocido: {s}").ConfigureAwait(false);
            return;
        }
        ...
        await ReplyAsync($"{b.Connection.Name} ha reiniciado la posición del joystick.").ConfigureAwait(false);
    }
}
